  ...
  togglePlayButtonState(pauseAutoplay) {
    if (pauseAutoplay) {
      ...
      this.sliderAutoplayButton.setAttribute(
        "aria-label",
        window._UMG.shopSettings.accessibilityStrings.playSlideshow
      );
    } else {
      this.sliderAutoplayButton.classList.remove("slideshow__autoplay--paused");
      this.sliderAutoplayButton.setAttribute(
        "aria-label",
        window._UMG.shopSettings.accessibilityStrings.pauseSlideshow
      );
    }
  }
...
class VariantSelects extends HTMLElement {
  ...
  updateShareUrl() {
    ...
    shareButton.updateUrl(
      `${window._UMG.shopSettings.shopUrl}${this.dataset.url}?variant=${this.currentVariant.id}`
    );
  }
  ...
  setInputAvailability(listOfOptions, listOfAvailableOptions) {
    listOfOptions.forEach((input) => {
      if (listOfAvailableOptions.includes(input.getAttribute("value"))) {
        ...
        input.innerText =
          window._UMG.shopSettings.variantStrings.unavailable_with_option.replace(
            "[value]",
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        this.toggleAddButton(
          ...
          window._UMG.shopSettings.variantStrings.soldOut
        );
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    if (disable) {
      ...
      addButtonText.textContent =
        window._UMG.shopSettings.variantStrings.addToCart;
    }
  ...
  setUnavailable() {
    ...
    if (!addButton) return;
    addButtonText.textContent =
      window._UMG.shopSettings.variantStrings.unavailable;
